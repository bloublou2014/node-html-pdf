...
function getContent (page) {
  return page.evaluate(function () {
    ...
    var body
    var $body = document.getElementById('pageContent') || document.body
    if ($body) {
      body = $body.outerHTML
    }
...
// ------------------------------------
function definePaperOrientation (content, options) {
  var paper = {border: options.border || '0'}
  if (options.orientationFromContentSize) {
    paper.width = content.bodyWidth
    paper.height = content.bodyHeight
...
function definePaperSize (content, options) {
  var paper = definePaperOrientation(content, options)
